...
#     """Ticket status codes - Open = NOT (5 or 8)"""
#     NEW = 1
#     IN_PROGRESS = 2
#     CUSTOMER_NOTE = 3
#     WAITING_APPROVAL = 4
...
#         names = {
#             1: "New", 2: "In Progress", 3: "Customer Note", 4: "Waiting Approval",
#             5: "Complete", 6: "Waiting Customer", 7: "Waiting Vendor",
#             8: "Cancelled", 9: "Scheduled"
#         }
#         return names.get(status_code, f"Status {status_code}")
    
#     @classmethod
#     def is_open(cls, status_code: int) -> bool:
#         """Status NOT 5 (Complete) AND NOT 8 (Cancelled) = OPEN"""
#         return status_code not in [5, 8]
...
#         names = {4: "Critical", 1: "High", 2: "Medium", 3: "Low"}
...
# - OPEN: Any status NOT 5 (Complete) 
# - CLOSED: Status 5 (Complete) 
...
# Respond with JSON:
# {
#   "action": "count_entities" | "list_entities" | "count_tickets" | "search_tickets" | "aggregate_tickets" | "search_resources" | "search_contacts" | "semantic_search" | "aggregate_time",
#   "params": {
#     "company_id": <number>,
#     "company_name": "<string>",
#     "status": <number>,
...
#   },
#   "aggregation": {
#     "group_by": ["status", "priority", "company_name", "assigned_resource_name", "contact_name"]
#   },
#   "time_aggregation": {
...
# Status: 1=New, 2=In Progress, 3=Customer Note, 4=Waiting Approval, 5=Complete, 6=Waiting Customer, 7=Waiting Vendor, 8=Cancelled, 9=Scheduled
# Priority: 1=Critical, 2=High, 3=Medium, 4=Low

# EXAMPLES:
# COUNTING ENTITIES (IMPORTANT!):
# - "How many technicians?" → {"action": "count_entities", "entity": "resources"}
# - "How many resources are there?" → {"action": "count_entities", "entity": "resources"}
# - "How many active technicians?" → {"action": "count_entities", "entity": "resources", "params": {"is_active": true}}
# - "How many contacts?" → {"action": "count_entities", "entity": "contacts"}
# - "How many companies?" → {"action": "count_entities", "entity": "companies"}

# TIME QUERIES (IMPORTANT!):
# - "Which ticket took the most time?" → {"action": "aggregate_time", "time_aggregation": {"group_by": "ticket_id", "limit": 10}}
# - "Which ticket has maximum hours?" → {"action": "aggregate_time", "time_aggregation": {"group_by": "ticket_id", "limit": 1}}
# - "Most time consuming tickets?" → {"action": "aggregate_time", "time_aggregation": {"group_by": "ticket_id", "limit": 10}}
# - "Total hours by technician?" → {"action": "aggregate_time", "time_aggregation": {"group_by": "resource_id", "limit": 10}}

# LISTING ENTITIES:
# - "List all technicians" → {"action": "list_entities", "entity": "resources"}
# - "Show all resources" → {"action": "list_entities", "entity": "resources"}
# - "List contacts" → {"action": "list_entities", "entity": "contacts"}

# SEARCHING BY NAME:
# - "Find technician John" → {"action": "search_resources", "search_text": "John"}
# - "Find contact Sarah" → {"action": "search_contacts", "search_text": "Sarah"}
# - "Search for Mike" → {"action": "search_resources", "search_text": "Mike"}

# TICKETS:
# - "How many open tickets?" → {"action": "count_tickets", "params": {"is_open": true}}
# - "Show tickets assigned to John Smith" → {"action": "search_tickets", "params": {"assigned_resource_name": "John Smith"}}
# - "Who has the most tickets?" → {"action": "aggregate_tickets", "aggregation": {"group_by": ["assigned_resource_name"]}}

# SEMANTIC SEARCH (for content/expertise):
# - "What is Uttam's last name?" → {"action": "semantic_search", "search_params": {"query": "Uttam", "tables": ["contacts", "resources"], "limit": 5}}
# - "Tickets about WiFi" → {"action": "semantic_search", "search_params": {"query": "WiFi problems", "tables": ["tickets"], "limit": 10}}
# - "Who knows Exchange Server?" → {"action": "semantic_search", "search_params": {"query": "Exchange Server", "tables": ["resources"], "limit": 5}}
# """
...
#             if params["is_open"]:
#                 query = query.not_.in_("status", [5, 8])  # Open tickets
#             else:
#                 query = query.in_("status", [5, 8])  # Closed tickets
...
#             filters.append(f"status={TicketStatus.get_name(params['status'])}")
        
#         if params.get("priority") is not None:
#             filters.append(f"priority={TicketPriority.get_name(params['priority'])}")
...
#         """Enhance results - names are already in the data!"""
        
#         # Add status/priority names
...
#                     "title": t.get("title"),
#                     "status": TicketStatus.get_name(t.get("status", 0)),
#                     "priority": TicketPriority.get_name(t.get("priority", 0)),
...
# # ==================== MAIN SERVICE ====================
# class AIService:
#     """Improved AI service with full resource/contact support + vector search"""
    
#     def __init__(self):
#         self.client = AsyncOpenAI(api_key=settings.openai_api_key)
#         self.db_service = get_database_service()
#         self.filter_builder = QueryFilterBuilder(self.db_service.client)
#         self.enhancer = ResultEnhancer(self.db_service.client)
#         self.summary = SummaryGenerator(self.client)
        
#         # NEW: Try to load embedding service for vector search
#         try:
...
#             "aggregate_time": self._aggregate_time,  # NEW: Time tracking
...
#         """Aggregate tickets - now supports name-based grouping!"""
#         params = ai_response.get("params", {})
...
#         # Fetch all tickets with grouping fields
...
#         """Aggregate time entries - NEW for time tracking queries"""
...
#             # Enrich with ticket/resource names
...
#         """Search tickets - supports name-based search!"""
#         params = ai_response.get("params", {})
...
# ==================== MAIN SERVICE ====================
class AIService:
    """Improved AI service with full resource/contact support + vector search + lookup tables"""
