...
SOLUTION QUERIES (NEW - for finding ticket resolutions):
- "Find solution for ticket [NUMBER]" → use get_solution with ticket_number
- "What's the solution for ticket [NUMBER]?" → use get_solution with ticket_number
- "Show me resolution for ticket [NUMBER]" → use get_solution with ticket_number
- "How was ticket [NUMBER] resolved?" → use get_solution with ticket_number
- "Solution to ticket [NUMBER]" → use get_solution with ticket_number
- "Get solution for [NUMBER]" → use get_solution with ticket_number
...
Respond with JSON:
{
  ...
  "params": {
    "ticket_number": "<string - for get_solution action>",
    "ticket_id": <number - for get_solution action>,
    "company_id": <number>,
    "company_name": "<string>",
    "status": <number>,
    "priority": <number>,
...
class AIService:
    ...
    async def _get_solution(self, ai_response: Dict) -> Dict:
        """Get solution for a specific ticket"""
        params = ai_response.get("params", {})
        ...
        if not ticket_number and not ticket_id:
            return {
                "answer": "Please provide a ticket number or ticket ID to find the solution.",
                "tickets": [],
                "ticket_count": 0
            }

        try:
            # Query by ticket number or ID
            ...
            if not result.data or len(result.data) == 0:
                return {
                    "answer": f"Ticket {ticket_number or ticket_id} was not found in the database.",
                    "tickets": [],
                    "ticket_count": 0
                }
            ...
            if not resolution or resolution.strip() == "":
                return {
                    "answer": f"Ticket {ticket.get('ticket_number', ticket_id)} (ID: {ticket.get('id')}) - {ticket.get('title', 'No title')}\n\nThis ticket does not have a solution/resolution recorded yet.",
                    "tickets": [ticket],
                    "ticket_count": 1
                }
...
"""
            ...
            return {
                "answer": f"Error retrieving solution: {str(e)}",
                "tickets": [],
                "ticket_count": 0
            }
